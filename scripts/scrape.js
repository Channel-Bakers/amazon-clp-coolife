		...
		for (let i = 0; i < CHUNKED_ASIN_LIST.length; i++) {
			try {
				...
				if (
					...
					fs.mkdirSync(`./data/asins/${DATE_PATH}`);
                }
                
                if (
                    !fs.existsSync(`./data/asins/${DATE_PATH}/${type}`)
                ) {
					fs.mkdirSync(`./data/asins/${DATE_PATH}/${type}`);
                }

				const fileData = fs.existsSync(
					`./data/asins/${DATE_PATH}/${type}/${FILE_NAME}.json`
				)
					? fs.readFileSync(
							`./data/asins/${DATE_PATH}/${type}/${FILE_NAME}.json`,
							'utf8'
